...
alphase_unit::alphase_unit()
{
	...
	for (ush_i=0; ush_i<virtual_devices_max_count; ush_i++)
	{
		unit_type[ush_i]=255; //
		unit_subtype[ush_i]=255; //
		unit_ver[ush_i]=255; //
	}

	
	unit_type[0]=1; //Тип модуля. Не должен равняться 0. Иначе модуль не будет опрашиваться контроллером.
	unit_subtype[0]=0; //Подтип модуля (при наличии модификаций одного модуля в разных исполнениях)
	unit_ver[0]=1; //Версия модуля. Не должен равняться 0. Иначе модуль не будет адресоваться контроллером.	
	...
	virtual_devices_count=0; // Количество дополнительных вируальных устройств с адресами addr+1, addr+2 и т.д.
	virtual_device_num=0; // Номер виртуального модуля к которому сейчас пришёл запрос. Занчение вычисляется автоматически при поступлениее запроса к модулю и имеет значение от 0 до virtual_devices_count включительно.
};
...
void alphase_unit::serialEvent() {
  ...
  while (Serial.available()) 
  {
    ...
    if ((buff[3]==255)&&(buff[4]==255)) // Если есть 2 стартовых байта 255 и 255
    {
      ...
      {
        if ((buff[6]>=addr)&&(buff[6]<=addr+virtual_devices_count)) // Если адрес модуля совпадает
        {
					virtual_device_num=buff[6]-addr; // Вычисляем адрес вируального устройства к которому пришёл запрос
    ...
    if ((buff[3]>=addr) && (buff[3]<=addr+virtual_devices_count) && (buff[4]==3)) // Если первый символ - адрес, второй - команда чтения
    {
    ...
    if ((buff[0]>=addr) && (buff[0]<=addr+virtual_devices_count) && (buff[1]==16)) // Если первый символ - адрес, второй - команда записи
    {
...
void alphase_unit::func_ADNet()
{
  ...
  case 0: // Запрос типа и версия модуля
    ...
    buff[4]=unit_ver[virtual_device_num];
    buff[5]=unit_type[virtual_device_num];
		buff[6]=unit_subtype[virtual_device_num];
  ...
  if (digitalRead(jmp_pin[5])==LOW)
	...
	if (jmp_pin[6]!=0) // Джампер 6 не является обязательным и используется, если !==0
		if (digitalRead(jmp_pin[6])==LOW)
			addr+=64;    
	if (jmp_pin[7]!=0) // Джампер 7 не является обязательным и используется, если !==0
